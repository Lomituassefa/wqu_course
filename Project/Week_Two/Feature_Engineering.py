...
def get_default_file_path():
    ...
    return os.path.join(script_dir, "Data", "buenos-aires-real-estate-*.csv")

def merged_files(file_path=None):
...
def modify_cols(data):
    lat_lon_split = data["lat-lon"].str.split(",", expand=True)
    return (
        data
        .assign(
            lat=lat_lon_split[0].astype(float),
            lon=lat_lon_split[1].astype(float),
...
def clean_files(file_path=None):
    if file_path is None:
        file_path = get_default_file_path()
...
if __name__ == "__main__":
    ...
    plt.tight_layout()
    fig2.savefig(os.path.join(script_dir, "Outputs","Correlation matrix.png"), dpi=100)
    plt.close(fig2)